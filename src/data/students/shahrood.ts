...
import { SHAHROOD_PROFILES } from './shahrood/profiles';
import { SHAHROOD_OPT_RESULTS } from './shahrood/opt-results';
import { SHAHROOD_NARRATIVE_RESULTS } from './shahrood/narrative-results';

/**
 * تجمیع‌کننده داده‌های نرمالایزشده برای زبان‌آموزان شاهرود.
...
const optResultsMap = new Map(SHAHROOD_OPT_RESULTS.map(r => [r.studentId, r]));
const narrativeResultsMap = new Map(SHAHROOD_NARRATIVE_RESULTS.map(r => [r.studentId, r]));

// داده‌های تفکیک‌شده را به مدل نمایش `Student` تبدیل می‌کنیم
export const SHAHROOD_STUDENTS: Student[] = SHAHROOD_PROFILES.map(profile => {
