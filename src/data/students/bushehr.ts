...
import { BUSHEHR_PROFILES } from './bushehr/profiles';
import { BUSHEHR_OPT_RESULTS } from './bushehr/opt-results';
import { BUSHEHR_NARRATIVE_RESULTS } from './bushehr/narrative-results';

/**
 * تجمیع‌کننده داده‌های نرمالایزشده برای زبان‌آموزان بوشهر.
...
const optResultsMap = new Map(BUSHEHR_OPT_RESULTS.map(r => [r.studentId, r]));
const narrativeResultsMap = new Map(BUSHEHR_NARRATIVE_RESULTS.map(r => [r.studentId, r]));

// داده‌های تفکیک‌شده را به مدل نمایش `Student` تبدیل می‌کنیم
export const BUSHEHR_STUDENTS: Student[] = BUSHEHR_PROFILES.map(profile => {
